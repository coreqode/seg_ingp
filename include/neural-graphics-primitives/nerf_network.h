...
__global__ void extract_mask(
	...
	const uint32_t i = threadIdx.x + blockIdx.x * blockDim.x;
	if (i >= n_elements) return;
	seg[i*seg_stride] = rgbd[i*rgbd_stride];
}
...
__global__ void pack_mask_with_density(
	...
	rgbd[i * rgbd_stride] = density[i * density_stride];
	rgbd[i * rgbd_stride + 1] = seg[i * seg_stride];
}
...
public:
	...
	std::unique_ptr<tcnn::Context> forward_impl(cudaStream_t stream, const tcnn::GPUMatrixDynamic<float>& input, tcnn::GPUMatrixDynamic<T>* output = nullptr, bool use_inference_params = false, bool prepare_input_gradients = false) override {
		...
		if (output) {
			...
			// forward->seg_network_output = tcnn::GPUMatrixDynamic<T>{output->data() + 4, m_rgb_network->padded_output_width(), batch_size, output->layout()};
		}
		...
		if (train_seg){
			forward->seg_network_input = forward->density_network_output.slice_rows(0, m_seg_network_input_width);
			forward->seg_network_ctx = m_seg_network->forward(stream, forward->seg_network_input, &forward->seg_network_output, use_inference_params, prepare_input_gradients); 
	...
	void backward_impl(
		...
		// debug_print(dL_dseg, dL_dseg.n_bytes(), 0, 1, 16);
