...
__device__ float network_to_mask(float val, ENerfActivation activation) {
	switch (activation) {
		case ENerfActivation::None: return val;
		case ENerfActivation::ReLU: return val > 0.0f ? val : 0.0f;
		case ENerfActivation::Logistic: return tcnn::logistic(val);
...
__device__ LossAndGradient loss_and_gradient(const vec3& target, const vec3& prediction, ELossType loss_type) {
	switch (loss_type) {
		...
		case ELossType::BCE:       return binary_cross_entropy(target, prediction); break;
...
__global__ void generate_training_samples_nerf(
	...
	// printf("\n ray_d_normalized %f %f %f", ray_d_normalized.x, ray_d_normalized.y, ray_d_normalized.z);
	// printf("\n warped_dir %f %f %f", warped_dir.x, warped_dir.y, warped_dir.z);
	t=startt;
	j=0;
...
void Testbed::update_density_grid_nerf(float decay, uint32_t n_uniform_density_grid_samples, uint32_t n_nonuniform_density_grid_samples, cudaStream_t stream) {
	...
	for (uint32_t i = 0; i < n_steps; ++i) {
		...
		for (size_t i = 0; i < n_density_grid_samples; i += batch_size) {
			...
			// debug_print<network_precision_t>(mlp_out+i, 32 * sizeof(network_precision_t), 32 );
...
void Testbed::train_nerf_step(uint32_t target_batch_size, Testbed::NerfCounters& counters, cudaStream_t stream) {
	...
	// debug_print<network_precision_t>(mlp_out, 32*sizeof(network_precision_t), 1, 32);
	...
	auto hg_enc = dynamic_cast<GridEncoding<network_precision_t>*>(m_encoding.get());
		...
		//coords are position, dt(diference between sample in direction of ray), direction
		...
		// debug_print<float>(coords_matrix, coords_matrix.n_bytes(),  0, 1, 7);
		// debug_print<float>(coords_matrix, coords_matrix.n_bytes(),  7, 1, 7);
		// debug_print<float>(coords_matrix, coords_matrix.n_bytes(),  14, 1, 7);
		// printf("\n size of coords matrix: %d x %d\n", coords_matrix.rows(), coords_matrix.cols());
		// printf("\n matrix layout: %d\n", coords_matrix.layout() == tcnn::CM ? 1: 0);
		// exit(0);
	...
	// printf("\n Memory address of compacted rgbsigma matrix: %p\n", &compacted_rgbsigma_matrix);
	// printf("\n Memory address of rgbsigma matrix: %p\n", &rgbsigma_matrix);
	// printf(" ------------------------------------------------------- ");
